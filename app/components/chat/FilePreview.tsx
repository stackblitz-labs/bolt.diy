      ...
      {files.map((file, index) => (
        <div key={file.name + file.size} className="mr-2 relative">
          {imageDataList[index] ? (
            ...
            <div className="relative w-32 h-20 rounded-lg border border-bolt-elements-borderColor bg-bolt-elements-background-depth-2 p-2 flex flex-col justify-between">
              <div className="flex items-center gap-2 text-bolt-elements-textSecondary text-xs">
                <div className="i-ph:file-text text-base" />
                <span className="truncate">{file.name}</span>
              </div>
              <div className="text-[10px] text-bolt-elements-textTertiary">{Math.round(file.size / 1024)} KB</div>
              <button
                onClick={() => onRemove(index)}
                className="absolute -top-1 -right-1 z-10 bg-black rounded-full w-5 h-5 shadow-md hover:bg-gray-900 transition-colors flex items-center justify-center"
              >
