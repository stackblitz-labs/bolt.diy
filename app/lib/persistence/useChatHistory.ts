...
${value.content}
      ...
      try {
        ...
        if (projectId && isUserAuthenticated()) {
          try {
            ...
            console.log('Snapshot saved to server', { projectId });
          } catch (serverError) {
            console.warn('Failed to save snapshot to server, client-only backup exists', {
              projectId,
  ...
  return {
    ...
    storeMessageHistory: async (messages: Message[]) => {
      ...
      if (projectId && isUserAuthenticated()) {
        try {
          await setServerMessages(projectId, [...archivedMessages, ...messages]);
          console.log('Messages saved to server', { projectId });
        } catch (error) {
